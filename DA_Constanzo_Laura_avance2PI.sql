...
SELECT * FROM Categorias; -- Visualizo la tabla para ver como quedó
...
UPDATE Productos -- Actualiza la tabla de Productos
SET Precio = Precio + 1 -- Aumentar 1 al valor actual de Precios
...
SELECT * FROM Productos; -- Es FK, da error y no se borra para no generar inconsistencias en las tablas
...
DELETE FROM Empleados WHERE SucursalID = 10; -- No se borra ninguna, no hay ese ID en sucursales 
...
SELECT COUNT(OrdenID) as CantidadTotal -- Ejecuto COUNT no en * para evitar errores y sobrecarga, sino en la PK
FROM Ordenes;

-- CONSULTA 2: ¿Cuántos empleados existen en cada departamento?

SELECT * FROM Empleados; -- Visualizo la tabla completa para ver donde me encuentro
...
/* CONSULTA 5:¿Cuáles son las sucursales (dimensión) con un promedio de ventas por orden superior a un valor específico (métrica), 
 ordenadas por el promedio de kilómetros recorridos para las entregas de mayor a menor (métrica)? */

SELECT * FROM Ordenes;

SELECT SucursalID, 
AVG(TotalCompra) as PromedioVentaporOrden,
AVG(KilometrosRecorrer) as PromedioKM
FROM Ordenes
GROUP BY SucursalID -- Porque SucursalID es una dimensión
HAVING AVG(TotalCompra) > 50 -- Luego de GROUP BY
ORDER BY PromedioKM DESC; -- Por default es ASC
